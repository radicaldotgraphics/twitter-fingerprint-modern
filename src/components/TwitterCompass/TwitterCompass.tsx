...
export default function TwitterCompass({ data }: TwitterCompassProps) {
  ...
  }, [showChart, activeChart, data, renderCharts]);

  const renderCharts = () => {
    // Clear all canvases
    [backgroundRef, characterCountsRef, timeOfDayRef, mostUsedRef, mostUsedMarkersRef, topLayerRef].forEach(ref => {
      if (ref.current) {
  ...
  const renderTimeOfDayChart = () => {
    ...
    const hours = Object.keys(timeData).map(Number).sort((a, b) => a - b);
    
    hours.forEach((hour) => {
      const angle = (hour * 15 - 90) * Math.PI / 180; // 15 degrees per hour
      const radius = minRadius + (timeData[hour] / maxValue) * (maxRadius - minRadius);
  ...
  return (
    <div className="twitter-compass">
      <div className={`chart-inner ${showChart ? 'active' : ''}`}>
        <canvas ref={backgroundRef} id="background" width="600" height="538"></canvas>
        <canvas 
        ...
        ></canvas>
        <canvas ref={mostUsedMarkersRef} id="most-used-markers" width="600" height="538"></canvas>
        <canvas ref={topLayerRef} id="top-layer" width="600" height="538"></canvas>
      </div>
      ...
      <div className={`info ${showInfo ? 'active' : ''}`}>
        <div id="stats">
          ...
          <div className="stat-wrap">
            <p className="intro">Toggle data sets by the following parameters.</p>
            <p className={`stat time-of-day ${activeChart === 'time-of-day' ? 'active' : ''}`}>
              Most active at <span className="most-active">{data.stats.mostActiveTime}</span>
              <span className="divider">-</span>Least active at <span className="least-active">{data.stats.leastActiveTime}</span>
            </p>
            <p className={`stat tweet-length ${activeChart === 'length' ? 'active' : ''}`}>
              Average Tweet length <span>{data.stats.averageTweetLength} characters</span>
            </p>
            <p className={`stat most-used ${activeChart === 'freq' ? 'active' : ''}`}>
        ...
        <h2>About.</h2>
        <p className="about">
          This is the result of an exercise we did in our spare time; we call it Twitter Compass. The objective was to build a project that we could complete quickly and build in iterations. We set aside some time and went from ideation to design to development inside of a week.
          <br /><br />
          The resulting chart is a type of Twitter fingerprint: your active times + how wordy you are can be compared to others. Additionally, we noticed that an authentic user posts much more evenly throughout the day, but a bot or paid intern has their tweets scheduled at specific times.
          <br /><br />
          This was a fun, quick project. There are more robust Twitter visualization tools that exist, but there are none that are prettier.
        </p>
        <footer>
